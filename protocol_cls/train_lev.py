...
def main(i, flow_dict):
	f = open('results_lev/results_%d.txt'%i, 'w')
	...
	for epoch_i in trange(5, mininterval=2, \
		...
		with open('results_lev/atten_%d.txt'%i, 'w') as f2:
			f2.write(' '.join(map('{:.4f}'.format, score)))

		# write F1, PRECISION, RECALL
		with open('results_lev/metric_%d.txt'%i, 'w') as f3:
			f3.write('F1 PRE REC\n')
		...
		with open('results_lev/cm_%d.pkl'%i, 'wb') as f4:
